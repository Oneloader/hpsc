...
class FileController extends Controller
{
    ...
    /**
     * 上传图片
     * @param string $flag 兼容markdown编辑器上传图片，返回特定格式   @author szh szh@ourstu.com
     * @author huajie <banhuajie@163.com>
     */
    public function uploadPicture($flag = '')
    {
        ...
        /* 兼容markdown编辑器返回特定格式 */
        if ($flag == 'markdown') {
            $markdown['success'] = $return['status'] ;
            if($return['status']) {
                $markdown['url'] = $return['data']['file']['path'] ? $return['data']['file']['path'] : $return['data']['file']['url'] ;
            }else{
                $markdown['message'] = $return['info'] ;
            }
    ...
    public function markDownUpload() {
        $_FILES['file'] = $_FILES['editormd-image-file'] ;
        unset($_FILES['editormd-image-file']) ;
        $this->uploadPicture('markdown') ;
    }
}
