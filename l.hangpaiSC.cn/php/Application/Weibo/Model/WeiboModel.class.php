...
//    public function addWeibo($uid, $title, $content = '', $type = 'feed', $feed_data = array(), $from = '',$category,$images)
    public function addWeibo($uid, $title, $content = '',$category,$images,$belong)
    {
        ...
        //写入数据库
//        $data = array('uid' => $uid, 'title'=>$title, 'content' => $content, 'type' => $type, 'data' => serialize($feed_data), 'from' => $from,'category'=>$category,'images'=>$images);
        $data = array('uid' => $uid, 'title'=>$title, 'content' => $content,'category'=>$category,'images'=>$images,'belong'=>10002);
     ...
     * @return mixed
     */
    public function getWeiboListByIndex($param = null)
    {
        !empty($param['field']) && $this->field($param['field']);
        !empty($param['where']) && $this->where($param['where']);
        !empty($param['limit']) && $this->limit($param['limit']);
        !empty($param['order']) && $param['order'] = 'create_time desc';
        !empty($param['page']) && $this->page($param['page'], empty($param['count']) ? 5 : $param['count']);
        $this->order($param['order']);
        $list = $this->select();
        return $list;
    }
    ...
    public function getWeiboDetail1($id)
    {
        $weibo = $this
            ->field('id,uid,title,content,category,create_time,comment_count,likes_count,images')
        ...
        $weibo = array(
            'id' => intval($weibo['id']),
            'title' => strval($weibo['title']),
            'content' => strval($weibo['content']),
            'create_time' => date('Y-m-d H:i',$weibo['create_time']),
            'comment_count' => intval($weibo['comment_count']),
            'likes_count' => intval($weibo['likes_count']),
            'uid' => $weibo['uid'],
     ...
     * 获取该用户下所有作品
    */
    public function getUserWeibo($uid,$start,$listRows){
        if (!empty($uid)){
            $weibo = $this
