    ...
    <nav class="navbar navbar-static-top">
        <div class="collapse navbar-collapse navbar-collapse-example">
            ...
            <ul class="nav navbar-nav navbar-right">
                <?php if(file_exists('m/index.php')){ $url_m='http://'.$_SERVER['HTTP_HOST'].U(admin/index/index); $url_m=str_replace('index.php?s=','m/index.php?s=',$url_m) ?>
                <li><a  target="_blank" href="<?php echo ($url_m); ?>">管理微社区</a> </li>
                <?php } ?>
...
<script>
    ...
    <div class="main-title">
        <h2>
            <?php if(isset($data)): ?>[ <?php echo ($data["title"]); ?> ] 子<?php endif; ?>
            <?php echo L('_MENU_MANAGER_');?>
        </h2>
    </div>

    <div class="clearfix row with-padding">
        <div class="col-xs-10">
            <a class="btn btn-default" href="<?php echo U('add',array('pid'=>I('get.pid',0)));?>"><?php echo L('_ADD_WITH_SPACE_');?></a>
            <button class="btn ajax-post confirm" url="<?php echo U('del');?>" target-form="ids"><?php echo L('_DELETE_WITH_SPACE_');?></button>
            <a class="btn btn-default" href="<?php echo U('import',array('pid'=>I('get.pid',0)));?>"><?php echo L('_IMPORT_');?></a>
            <button class="btn list_sort" url="<?php echo U('sort',array('pid'=>I('get.pid',0)),'');?>"><?php echo L('_SORT_');?></button>
        </div>
        <!-- 高级搜索 -->
        <div class="col-xs-2">
            <div class="search-form fr cf">
                <div class="sleft">
                    <div class="input-group">
                        <input type="text" name="title" class="form-control" value="<?php echo I('title');?>"
                               placeholder="<?php echo L('_MENU_NAME_INPUT_');?>">
                               <span class="input-group-btn">
                    <a class="btn btn-default" href="javascript:;" id="search" url="/admin.php?s=/menu/index.html"><i
        ...
        <form class="ids">
            <table class="table table-bordered table-striped table-hover">
                ...
                <tr>
                    ...
                    <th><?php echo L('_MENU_SUPERIOR_');?></th>
                    <th><?php echo L('_GROUP_');?></th>
                    <th><?php echo L('_URL_');?></th>
                    <th><?php echo L('_SORT_');?></th>
                    <th><?php echo L('_DEV_MODE_ONLY_');?></th>
                    <th><?php echo L('_HIDE_');?></th>
                ...
                <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ((isset($menu["up_title"]) && ($menu["up_title"] !== ""))?($menu["up_title"]):L('_NONE_')); ?></td>
                            <td><?php echo ($menu["group"]); ?></td>
                            <td><?php echo ($menu["url"]); ?></td>
                            <td><?php echo ($menu["sort"]); ?></td>
                            <td>
                                <a href="<?php echo U('toogleDev',array('id'=>$menu['id'],'value'=>abs($menu['is_dev']-1)));?>"
                                   class="ajax-get">
                                    <?php echo ($menu["is_dev_text"]); ?>
                                </a>
                            </td>
                            <td>
                                <a href="<?php echo U('toogleHide',array('id'=>$menu['id'],'value'=>abs($menu['hide']-1)));?>"
                                   class="ajax-get">
                                    <?php echo ($menu["hide_text"]); ?>
                                </a>
                            </td>
                            <td>
                                <a title="<?php echo L('_EDIT_');?>" href="<?php echo U('edit?id='.$menu['id']);?>"><?php echo L('_EDIT_');?></a>
                                <a class="confirm ajax-get" title="<?php echo L('_DELETE_');?>" href="<?php echo U('del?id='.$menu['id']);?>"><?php echo L('_DELETE_');?></a>
                            </td>
                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <?php else: ?>
                    <td colspan="10" class="text-center"><?php echo L('_ADMIN_USER_EMPTY_');?></td><?php endif; ?>
                </tbody>
            </table>
        </form>
        <!-- 分页 -->
