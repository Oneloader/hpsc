...
<script>
    ...
    <div class="clearfix">
        <div class="col-xs-9 pd0">
            ...
            <button class="btn ajax-post" url="<?php echo U('User/changeStatus',array('method'=>'forbidUser'));?>"
                    target-form="ids" data-forbid="forbid"><?php echo L('_DISABLE_');?>
            </button>
    ...
    <div class="data-table with-padding adminlte-os-table">
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
            <!--<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>-->
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($vo['status'] == 1 AND $vo['alive_line'] < time()): ?><a href="javascript:void(0)"
                                   class="ajax-get" data-role="forbiduser" data-value="<?php echo ($vo["uid"]); ?>"><?php echo L('_DISABLE_');?></a>
                            <?php else: ?>
                                <a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>"
                                   class="ajax-get"><?php echo L('_ENABLE_');?></a>
                                <?php if($vo['status'] == 1): ?><span style="color: red;font-size: 12px;">禁用至：<?php echo (date('Y-m-d H:i:s',$vo['alive_line'])); ?></span><?php endif; endif; ?>
                        ...
                        <td>
                            <a href="<?php echo U('AuthManager/changeToManager?uid='.$vo['uid']);?>" class="authorize">
                                成为管理员
                            </a>
                        </td>
                        ...
                        <td><?php echo ($vo["mark"]["from"]); ?></td>
                        <td><?php echo ($vo["mark"]["type"]); ?></td>
...
//        nav_obj.trigger("click");
    ...
    <script src="/Public/zui/js/zui.min.js"></script>
    <script type="text/javascript">
        //选择搜索方式
