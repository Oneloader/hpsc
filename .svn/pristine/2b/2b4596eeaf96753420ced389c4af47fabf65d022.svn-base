...
class ManagerController extends AdminController
{
    ...
    {
        $this->issueModel = D('Manager/Manager');
        parent::_initialize();
    }
...
//        $map['status'] = array('egt', 0);
        ...
        $this->meta_title = L('_MANAGER_INFO_');
        $this->display();
    }
    ...
    public function expandinfo_details($uid = 0)
    {
        if (IS_POST) {
            ...
            //同时修改member表生日字段值
            $user['birthday'] = date("Y-m-d", text($_POST['生日']));
    ...
    public function changeStatus($method = null)
    {
        ...
        switch (strtolower($method)) {
            ...
            case 'resumeuser':
                ...
                M('ucenter_member')->where($map1)->save($data);
                $data['alive_line'] = -1;
                $this->resume('Member', $map, array('success' => '状态恢复成功！', 'error' => '状态恢复失败！'),$data);
                break;
            case 'deleteuser':
                $data = array('status' => -1);
                M('ucenter_member')->where($map1)->save($data);
    ...
    public function bindWeChat()
    {
        ...
        foreach ($list as &$val) {
            $val['id'] = $val['uid'];
            if(M('sync_login')->where(array('uid' => $val['uid']))->find()) {
                $val['wechat'] = 1;
            } else {
                $val['wechat'] = 0;
            }
        ...
        $builder = new AdminListBuilder;
        $builder->meta_title = '微信绑定';
        $builder->title('微信绑定');
        $builder->button('解绑', array('class' => 'btn ajax-post confirm', 'url' => U('User/delwechatbind'), 'target-form' => 'ids'))
            ->keyId('uid', 'UID')
            ->keyText('nickname', '昵称')
            ->keyYesNo('wechat', '是否微信绑定')
            ->keyDoAction('User/delwechatbind?ids=###', '解绑')
            ->data($list)
            ->display();
    }

    public function delWeChatBind($ids)
    {
        $ids = is_array($ids) ? $ids : explode(',', $ids);
        $res = D('sync_login')->where(array('uid' => array('in', $ids)))->delete();
        if($res) {
            $this->success('解绑成功', U('User/bindwechat'));
        } else {
            $this->error('解绑失败');
        }
    }
    ...
    public function ForbidUser() {
        ...
        if(IS_POST){
            ...
            $day = I('day', -1, 'string') ;
            if(!is_numeric($day)) {
                $this->error("输入的禁用天数有误~") ;
            }
            if($day == 0){
                $doTime = 0 ;
            }else{
                $doTime = round(time() + 24*60*60*$day) ;
            }
            $doForbid = $this->doForbid($id, $doTime) ;
            if($doForbid['status']){
                $this->success("禁用用户成功~") ;
            }else{
                $this->error($doForbid['info']) ;
            }
        }
        $users = array() ;
        if(is_array($id)){
            foreach ($id as $val) {
                $users[] = query_user(array('nickname','uid'), $val) ;
            }
            unset($val) ;
        }
        $this->assign('hasAdmin', $hasAdmin);
    ...
    private function doForbid($id, $time) {
        if($time) {
            $map['uid'] = array('in', $id);
            $map1['id'] = array('in', $id);
            $data['status'] = 1;
            $res = M('ucenter_member')->where($map1)->save($data);
            if($res === false){
                $this->error("禁用用户表失败~") ;
            }
            $data['alive_line'] = $time;
            $this->resume('Member', $map, array('success'=>'禁用用户成功~', 'error'=>'禁用会员用户表失败~'), $data);
        }else{//永久封禁
            $map['uid'] = array('in', $id);
            $map1['id'] = array('in', $id);
            $data = array('status' => 0);
