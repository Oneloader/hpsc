...
class AdminListBuilder extends AdminBuilder
{
    ...
    public function buttonDisable($url = null, $title = '禁用', $attr = array())
    {
        ...
        return $this->buttonSetStatus($url, 2, $title, $attr);
    }

    public function buttonEnable($url = null, $title = '审核', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
    ...
    public function buttonAdd($url = null, $title = '新增', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
        $attr['class']='btn ajax-post btn-info';
        return $this->buttonSetStatus($url, 1, $title, $attr);
...
//            $this->setSearchPostUrl('');
    ...
    {
        $map = array(-1 => '删除', 0 => '未审核', 1 => '已审核', 2 => '禁用');
        return $this->key($name, $title, 'status', $map);
    }

    public function keyIsTop($name = 'is_top', $title = '推荐')
    {
        $is_top = D()->where()->select();
        $map = array('' => '未推荐', 1 => L('_STICK_'));
        return $this->key($name, $title, 'is_top', $map);
    }

    public function keyCategory($name = 'category', $title = '微博分类')
    {
        ...
        return $this->key($name, $title, 'category', $map);
    }

    public function keyCate($name, $title)
    {
        return $this->key($name, $title, 'cate');
    }

    public function keyRecommend($name, $title, $opt = null)
    {
        return $this->key($name, $title, 'recommend',$opt);
    }

    public function keyPos($name, $title)
    {
        return $this->key($name, $title, 'belong');
    }

    public function keyMember($name = 'uid', $title = '用户')
    {
        $topic = D('member')
            ->where(['status'=>1])
            ->select();
        $categoty = [];
        foreach ($topic as $key=>$val){
            $categoty[$val['uid']] = $val['nickname'];
        }
//        var_dump($categoty);exit;
        $map = $categoty;

//        return $this->key($name, $title, 'uid');
        return $this->key($name, $title, 'uid', $map);
    }
    ...
    public function display($solist = '')
    {
        ...
        $this->convertKey('cate', 'html', function ($value, $key, $item) {
            if (is_string($value)) {//value是图片id
                $cate = explode(',',$value);
                foreach ($cate as $val){
                    $cate_name = D('weibo_topic')
                        ->field('id,name')
                        ->where(['id'=>$val])
                        ->find();
                    $ca_name .= "<p>".$cate_name['name']."</p>";
                }
                $html = $ca_name;
            }
            return $html;
        ...
        $this->convertKey('recommend', 'html', function ($value, $key, $item) {
            if (is_string($value)) {//value是is_top id
                if (strpos($value,$key['opt']) !== false) {
                    $ca_name = "<span>已推荐</span>";
                } else {
                    $ca_name = "<span>未推荐</span>";
                }
                $html = $ca_name;
            }
            return $html;
        ...
        $this->convertKey('belong', 'html', function ($value, $key, $item) {
            if (is_string($value)) {//value是图片id
                $cate = explode(',',$value);
                foreach ($cate as $val){
                    $cate_name = D('adv_pos')
        ...
        //推荐
//        $this->convertKey('is_top', 'html', function ($value, $key, $item) {
//            if (is_string($value)) {//value是分组id
//                $cate = explode(',',$value);
//                foreach ($cate as $val){
//                    $cate_name = D('adv_pos')
//                        ->field('id,name,title')
...
//        });
        ...
        $this->assign('cssList', $this->_css);
        $this->assign('scriptList', $this->_script);
        /*加入搜索 陈一枭*/
     ...
     * @auth 施志宏
     */
    public function addScript($script_name) {
