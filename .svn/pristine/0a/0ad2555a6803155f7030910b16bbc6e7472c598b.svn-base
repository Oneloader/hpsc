 ...
 * @return string
 */
function get_short_sp($str, $num)
{
    if (utf8_strlen($str) > $num) {
        $tag = '...';
    }
    $str = get_short($str, $num) . $tag;
    return $str;
}
...
/**非正常状态资讯显示提示
 * @param array $info  资讯
 * @return array $info  增加提示后的资讯
 * @author:szh(施志宏) szh@ourstu.com
 */
function illegal_status($info) {
    $a_tag = '' ;
    if ($info['status'] == -1) {
        $a_tag = "【审核失败】";
    ...
    if ( $info['post_time'] > time() ) {
        $a_tag = "【预发布】" ;
    }
    $info['illegal_status'] = $a_tag ;
    return $info ;
}
 ...
 * @param $name
 * @param string $data
 * @param bool  $flag  输出不带  toolbars:[]  的数据
 * @return mixed
 * @author szh(施志宏) szh@ourstu.com
 */
function get_editor_config($name, $data='', $flag=false) {
    if($data != ''){
        preg_match_all('/toolbars:\[(.*)\]/', $data, $array) ;
        if($array[1][0]){
            $conf = $data ;
        }else{
            $conf = "toolbars:[".$data."]" ;
        ...
        $conf = "toolbars:[['source','|','bold','italic','underline','fontsize','forecolor','justifyleft','fontfamily','|','map','emotion','insertimage','insertcode']]" ;
        $conf = modC('DEFAULT', $conf, 'EDITORTOOL') ;
    }
    $name = strtoupper($name) ;
    $config = modC($name, '', 'EDITORTOOL') ;
    if($config == false){
        $map['name'] = '_'. 'EDITORTOOL' . '_' . $name ;
        $map['title'] = $name.'的编辑器配置信息' ;
        $map['create_time'] = time() ;
        $map['update_time'] = time() ;
        $map['status'] = 1 ;
        $map['value'] = trim(json_encode($conf), '"') ;
        D('config')->add($map) ;
    }else{
    ...
    if($flag) {
        preg_match_all('/toolbars:\[\[(.*)\]\]/', $conf, $search) ;
        if($search[1][0]){
            return $search[1][0] ;
        }
    }
...
/**获取用户配置的markdown编辑器配置
 * @param $name
 * @param string $data
 * @return mixed
 * @author szh(施志宏) szh@ourstu.com
 */
function get_markdown_config($name, $data='') {
    if($data != ''){
        $conf = $data ;
    }else{
        $conf = "width: \"100%\",
		...
		" ;
        $conf = modC('DEFAULT', $conf, 'MARKDOWN') ;
    }
    $name = strtoupper($name) ;
    $config = modC($name, '', 'MARKDOWN') ;
    if($config == false){
        $map['name'] = '_'. 'MARKDOWN' . '_' . $name ;
        $map['title'] = $name.'的MarkDown配置信息' ;
        $map['create_time'] = time() ;
        $map['update_time'] = time() ;
        $map['status'] = 1 ;
        $map['value'] = $conf ;
...
function is_m()
{
    $file = file_exists('./m/Conf/common.php');
    $is_m = false;
    if($file && is_mobile()) {
        $is_m = true;
    }
