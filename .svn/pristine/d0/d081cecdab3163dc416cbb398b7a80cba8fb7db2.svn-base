...
class AdminConfigBuilder extends AdminBuilder
{
     ...
     * @auth 陈一枭
     */
    public function keyHidden($name, $title, $subtitle = null,$options = null)
    {
        return $this->key($name, $title, $subtitle, 'hidden',$options);
    }
    ...
    public function keyMarkDown($name, $title, $subtitle = null, $config = '')
    {
        $key = array('name' => $name, 'title' => $title, 'subtitle' => $subtitle, 'type' => 'markdown', 'config' => $config);
        $this->_keyList[] = $key;
        return $this;
    }
     ...
     * @param $title
     * @param null $subtitle
     * @param array $config  $config['editor']=>富文本配置   $config['markdown']=>markdown编辑器配置  
     * @param array $style
     * @param string $type
     * @return $this
     * @author szh(施志宏) szh@ourstu.com
     */
    public function keyChooseEditor($name, $title, $subtitle=null, $config=array(), $style=array('width' =>'500px','height' =>'400px'), $type='editor') {
        $type = strtolower($type) ;
        if($type == 'editor') {
            if (empty($config['editor'])) {
                $toolbars = "toolbars:[['source','|','bold','italic','underline','fontsize','forecolor','justifyleft','fontfamily','|','map','emotion','insertimage','insertcode']]";
            }elseif ($config['editor'] == 'all') {
                $toolbars = 'all';
            }else{
                $toolbars = "toolbars:[[" . $config['editor'] . "]]";
            }
            $config['editor'] = $toolbars ;
        }
        $key = array('name' => $name, 'title' => $title, 'subtitle' => $subtitle, 'type' => 'chooseEditor', 'config' => $config, 'style' => $style, 'editor'=>$type);
        $this->_keyList[] = $key;
        return $this;
    }
}