...
class BaseController extends AddonsController
{
    ...
    public function login()
    {
        ...
        if(is_mobile() && ($type == 'weixin')) {
            ...
            $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$config['APP_ID']}&redirect_uri={$redirect}&response_type=code&scope=snsapi_userinfo&state=weixin#wechat_redirect";
            redirect($url);
            exit;
        } else {
    ...
    public function existLogin()
    {
        ...
        if (0 < $uid) { //UC登陆成功
            ...
            if ($this->doLogin($uid, $aRemember == 'on')) { //登陆用户
                $this->addSyncLoginData($uid);
                $this->success('登陆成功！', session('login_http_referer'));
            } else {
                $this->error($Member->getError());
