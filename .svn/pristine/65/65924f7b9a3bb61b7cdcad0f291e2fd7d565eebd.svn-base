...
class WeiboCommentModel extends Model
{
    ...
    public function addComment($uid, $weibo_id, $content, $comment_id,$status = '-1')
    {
        ...
        //写入数据库
        $data = array('uid' => $uid, 'content' => $content, 'weibo_id' => $weibo_id, 'to_comment_id' => $comment_id,'status'=>$status);
    ...
    public function getCommentList($weibo_id, $page = 1, $count, $show_more = 0)
    {
        $order = modC('COMMENT_ORDER', 0, 'WEIBO') == 1 ? 'create_time asc' : 'create_time desc';
        $comment = $this->where(array('weibo_id' => $weibo_id, 'status' => 1))->order($order)->page($page, $count)->field('id')->select();
        /*        if($page == 1){
        ...
        $list = array();
        foreach ($ids as $v) {
            $list[] = $this->getComment($v);
        }
        return $list;
    }

    public function getCommentId($weibo_id, $page = 1, $show_more = 0)
    {
        $order = modC('COMMENT_ORDER', 0, 'WEIBO') == 1 ? 'create_time asc' : 'create_time desc';
